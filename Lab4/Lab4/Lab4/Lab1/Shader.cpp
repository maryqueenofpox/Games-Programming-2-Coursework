...
/*/
Initiates shader used by mesh
*/
void Shader::init(const std::string& filename)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaders[0] = generateShader(LoadShader("..\\res\\shader.vert"), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = generateShader(LoadShader("..\\res\\shader.frag"), GL_FRAGMENT_SHADER); // create fragment shader
	...
	shaderError(program, GL_LINK_STATUS, true, "Error: Shader program linking failed"); // cheack for error

	glValidateProgram(program); //check the entire program is valid
	shaderError(program, GL_VALIDATE_STATUS, true, "Error: Shader program not valid");
...
GLuint Shader::generateShader(const std::string& text, unsigned int type)
{
	...
	shaderError(shader, GL_COMPILE_STATUS, false, "Error compiling shader!"); //check for compile error

	return shader;
}
/*/
Loads shader for 3D model mesh from local file
*/
...
/*/
Displays error message when issue occurs when loading or binding shader to mesh
*/
void Shader::shaderError(GLuint shader, GLuint flag, bool isProgram, const std::string& errorMessage)
{
	GLint success = 0;
	GLchar error[1024] = { 0 };
